...
// 设计系统契约(见 AGENTS.md「Design System Review Rules」)的机器强制规则。
...
const noInlineAppTokenStyle = {
  ...
  message:
    "禁止用内联 style={{}} 承载 --app-* 设计 token;改用 Tailwind 任意值类,如 className=\"text-[var(--app-text)]\"(--app-* 是颜色、背景和边框语义源)。仅当值是运行时动态计算(进度宽度/计算位移等)才允许内联,并加 // eslint-disable-next-line no-restricted-syntax -- 动态值 注明原因。存量已 baseline,清理后用 `npx eslint src --prune-suppressions` 收缩。",
};
...
const noRawButton = {
  ...
  message:
    "禁止手搓原生 <button>;改用 <Button>(@/components/ui/button)原语,它已 --app-* 主题化并内建 focus-visible:ring(见 AGENTS.md「Design System Review Rules」)。确需裸 <button>(图标位/整卡可点/特殊交互)时,自己补 --app-* 类 + 焦点态,并加 // eslint-disable-next-line no-restricted-syntax -- 原因 注明。存量已 baseline,清理后用 `npx eslint src --prune-suppressions` 收缩。",
};
