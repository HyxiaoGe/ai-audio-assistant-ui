...
export default function LoginModal({ isOpen, onClose, onLogin }: LoginModalProps) {
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    setTimeout(() => {
      const user = { name: "Sean", email: "sean@example.com" };

      localStorage.setItem("isAuthenticated", "true");
      localStorage.setItem("user", JSON.stringify(user));
      document.cookie = "mock_auth=1; path=/; max-age=2592000; samesite=lax";
  ...
  return (
    <>
      ...
      >
        {/* 登录模态框 */}
        <div 
          className="relative w-full max-w-md rounded-2xl p-12 shadow-2xl"
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 rounded-lg flex items-center justify-center transition-colors hover:bg-white/50"
            style={{ color: '#64748B' }}
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="text-center mb-12">
            {/* Logo 图标 */}
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4" style={{ background: 'linear-gradient(135deg, #3B82F6 0%, #0F172A 100%)' }}>
              <Mic className="w-6 h-6 text-white" />
            </div>
            
            {/* 产品名称 - Heading/H1 */}
            <h1 className="text-h1 mb-2" style={{ color: '#0F172A' }}>
              AI Audio Assistant
            </h1>
            
            {/* 副标题 - Body/Default */}
            <p className="text-body-default" style={{ color: '#64748B' }}>
              智能音视频内容理解助手
            </p>
          </div>
          ...
          <div className="space-y-4 mb-8">
            ...
            <button
              ...
              disabled={loading}
              className="w-full h-12 rounded-lg border border-[#E2E8F0] bg-white flex items-center justify-center gap-3 transition-colors hover:bg-[#F8FAFC] disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ fontWeight: 500, fontSize: '16px', color: '#0F172A' }}
            >
              {/* Google Logo SVG */}
            ...
            <button
              ...
              disabled={loading}
              className="w-full h-12 rounded-lg bg-[#24292F] flex items-center justify-center gap-3 transition-colors hover:bg-[#32383F] disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          {/* 底部协议文字 - Caption */}
          <p className="text-caption text-center" style={{ color: '#94A3B8' }}>
            登录即代表同意{' '}
