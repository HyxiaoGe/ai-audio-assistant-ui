...
export function EmailCodeLoginPanel({
  ...
  const dispatchFailure = (
    ...
    dispatch({ type: "REQUEST_FAILED", operation, requestId, failure, now: Date.now() })
    return failure
  }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (failure.code === "invalid_code" || failure.code === "code_expired") {
  ...
  const codeError = isEmailStep ? null : state.error
  ...
  const errorMessage = (failure: EmailCodeFailure | null): string | null => {
    ...
    const seconds = failure.code === "rate_limited"
      ...
      : undefined
    return t(`auth.emailCode.errors.${failure.code}`, { seconds: seconds ?? 1 })
  }
  ...
  return (
    <div className="flex flex-col gap-5" data-testid="email-code-login-panel">
      {isEmailStep ? (
        <>
          <Button
            type="button"
          ...
          <DialogHeader>
            <DialogTitle className="text-xl text-[var(--app-text)]">
              {t("auth.emailCode.title")}
            </DialogTitle>
            <DialogDescription className="leading-6 text-[var(--app-text-muted)]">
              {t("auth.emailCode.description")}
            </DialogDescription>
          </DialogHeader>
          <form className="flex flex-col gap-4" onSubmit={handleStart} noValidate>
            <div className="flex flex-col gap-2">
              <Label htmlFor="email-code-email" className="text-[var(--app-text)]">
                {t("auth.emailCode.emailLabel")}
              </Label>
              <Input
                ref={emailInputRef}
                id="email-code-email"
              ...
              />
              {emailErrorMessage ? (
                <p id={emailErrorId} role="alert" className="text-sm text-[var(--app-danger)]">
                  {emailErrorMessage}
                </p>
              ) : null}
            </div>
        ...
        <>
          ...
          <form className="flex flex-col gap-4" onSubmit={handleVerify}>
            <div className="flex flex-col gap-2">
              <Label htmlFor="email-code-value" className="text-[var(--app-text)]">
                {t("auth.emailCode.codeLabel")}
              </Label>
              <Input
                ref={codeInputRef}
                id="email-code-value"
              ...
              />
              {codeErrorMessage ? (
                <p id={codeErrorId} role="alert" className="text-sm text-[var(--app-danger)]">
                  {codeErrorMessage}
                </p>
              ) : null}
